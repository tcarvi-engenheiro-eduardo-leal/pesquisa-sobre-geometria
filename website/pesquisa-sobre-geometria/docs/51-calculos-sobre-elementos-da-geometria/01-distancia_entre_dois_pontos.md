...
- Como humanos, não percebemos a distância no espaco 4D. 
    ...
    - Então, para ter uma medida de distância entre os dois objetos, calculamos:
        ...
        - Normalização de uma coordenada:
            ...
            - Sobre o conhecimento do dado, pode-se também supor que o fato de um dado se localizar dentro de certo intervalo torna o dado mais representativo para o aumento da distância entre pontos; mas o fato do dado se localizar dentro de outra dispersão torna o dado menos representativo para o aumento da distância que se está calculando. Assim, a própria normalização pode ser modificada por um indicador interno de representatividade. Entre valores zero e um, o valor medido pode aumentar um pouco ou diminuir um pouco, devido esta escolha de representatividade. Mas não mudaremos aqui a representatividade interna do dado e desconsideraremos mudanças devido a representatividade.
    - Então, para se ter uma medida de distância entre os dois objetos, cujas coordenadas estão normalizadas, calculamos:
...
- Aplicação desta distância:
    ...
    - Esta fórmula pode ser muito útil para descrever e caracterizar a aproximação/distância entre objetos. Entretanto, o sistema pode ficar muito complexo se usamos milhares de coordenadas e se tentamos normalizar todas elas e ainda fornecer peso ponderado para cada uma. Se ainda usarmos indicadores de representatividade para cada medida, teremos um sistema bastante complexo.
    - Para a análise de tal sistema complexo, pode-se usar algoritmos para dar peso e para normalizar os valores das coordenadas.
    - Além disso, sistemas de IA podem ser usados para que se descubra, a partir da análise histórica, quais as coordenadas relevantes para descrever o sistema, e quais pesos ponderados devem ser usados para diferenciar a importância das coordenadas. Pode-se prever o comportamento de sistemas complexos, a partir desta fórmula. Basta que se debruce sobre os dados e se faça perguntas relevantes e criativas sobre o relacionamento entre os diferentes objetos do espaço estudado.
...
- Pode ser que a análise sistêmica requeira a inclusão do objeto "PIB" ou do objeto "Velocidade do Crescimento Econômico", por exemplo. Neste caso, deve-se usar outra ferramenta matemática. O cálculo da distância entre dois pontos executa um cálculo em específico: calcula a relação de aproximação entre dois objetos que possuem os mesmos tipos de coordenadas. 
- Mas esta constatação não deve ser considerada como algo que diminue a capacidade de análise sistêmica desta medida matemática. Pode-se considerar que todo o País é um ponto, com milhares de coordenadas. E pode-se considerar que o mesmo País, no dia seguinte, no espaço temporal, é um segundo ponto. Logo, pode-se analisar este dois estados temporais do País. E pode-se comparar as diversas medidas temporais feitas para estes diferentes pontos. Não se pode errar no entendimento da métrica e medir a distância entre pontos e segmentos de reta, por exemplo. Não se pode misturar "Partidos Políticos" com "PIB", como objetos que seriam indevidamente semelhantes. Mas pode-se ter criatividade para organizar e descrever os dados, depois usando esta ferramenta matemática para entender melhor os dados.
